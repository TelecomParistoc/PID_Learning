...
class Dynamic_header_file_handler : public Ostream_handler<Dynamic_header_file_handler<Filename_aggregator, trunc_if_exists> >
{
    ...
    public:
        ...
        {
            if(trunc_if_exists)
                _private_ostream = std::move(std::make_unique<std::ofstream>(new std::ofstream(Filename_aggregator::aggregate(""), std::ios::out | std::ios::trunc)));
            else
                _private_ostream = std::move(std::make_unique<std::ofstream>(new std::ofstream(Filename_aggregator::aggregate(""), std::ios::out | std::ios::app)));
