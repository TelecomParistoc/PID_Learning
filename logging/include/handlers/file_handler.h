...
std::unique_ptr<std::ofstream> File_handler_fail_behaviour<Filename, Behaviour_factory, Input_type_Factory, Bad_File_Behaviour, trunc_if_exists>::_private_ostream;
...
class Dynamic_header_file_handler : public Ostream_handler<Dynamic_header_file_handler<Filename_aggregator, trunc_if_exists> >
{
    ...
    public:
        ...
        {
            if(trunc_if_exists)
                _private_ostream = std::move(std::make_unique<std::ofstream>(new std::ofstream(Filename_aggregator::aggregate("").c_str(), std::ios::out | std::ios::trunc)));
            else
                _private_ostream = std::move(std::make_unique<std::ofstream>(new std::ofstream(Filename_aggregator::aggregate("").c_str(), std::ios::out | std::ios::app)));
...
std::unique_ptr<std::ofstream> Dynamic_header_file_handler<Filename_aggregator, trunc_if_exists>::_private_ostream;
...
std::unique_ptr<std::ofstream> Dynamic_header_file_handler_fail_behaviour<Filename_aggregator, Behaviour_factory, Input_type_Factory, Bad_File_Behaviour, trunc_if_exists>::_private_ostream;
