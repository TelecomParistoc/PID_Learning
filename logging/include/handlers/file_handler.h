...
class File_handler : public Ostream_handler<File_handler<Filename, trunc_if_exists> >
{
    public:
        ...
        {
            if(trunc_if_exists)
                _private_ostream = std::move(std::make_shared<std::ofstream>(new std::ofstream(Filename, std::ios::out | std::ios::trunc)));
            else
                _private_ostream = std::move(std::make_shared<std::ofstream>(new std::ofstream(Filename, std::ios::out | std::ios::app)));
    ...
    private:
        static std::shared_ptr<std::ofstream> _private_ostream;
};

template <const char* Filename, bool trunc_if_exists>
std::shared_ptr<std::ofstream> File_handler<Filename, trunc_if_exists>::_private_ostream;
...
class File_handler_fail_behaviour : public Ostream_handler<File_handler_fail_behaviour<Filename, Behaviour_factory, Input_type_Factory, Bad_File_Behaviour, trunc_if_exists> >
{
    ...
    public:
        static bool initialize()
        {
            ...
            {
                Ostream_parent::_ostream = _private_ostream.get();
                return true;
            }
        }
...
std::shared_ptr<std::ofstream> File_handler_fail_behaviour<Filename, Behaviour_factory, Input_type_Factory, Bad_File_Behaviour, trunc_if_exists>::_private_ostream;
...
class Dynamic_header_file_handler : public Ostream_handler<Dynamic_header_file_handler<Filename_aggregator, trunc_if_exists> >
{
    ...
    public:
        ...
        {
            if(trunc_if_exists)
                _private_ostream = std::move(std::make_shared<std::ofstream>(new std::ofstream(Filename_aggregator::aggregate(""), std::ios::out | std::ios::trunc)));
            else
                _private_ostream = std::move(std::make_shared<std::ofstream>(new std::ofstream(Filename_aggregator::aggregate(""), std::ios::out | std::ios::app)));
...
std::shared_ptr<std::ofstream> Dynamic_header_file_handler<Filename_aggregator, trunc_if_exists>::_private_ostream;
...
class Dynamic_header_file_handler_fail_behaviour : public Ostream_handler<Dynamic_header_file_handler_fail_behaviour<Filename_aggregator, Behaviour_factory, Input_type_Factory, Bad_File_Behaviour, trunc_if_exists> >
{
    ...
    private:
        static std::auto_ptr<std::ofstream> _private_ostream;
};
